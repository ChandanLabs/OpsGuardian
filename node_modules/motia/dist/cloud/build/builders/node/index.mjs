...
//#region src/cloud/build/builders/node/index.ts
var NodeBuilder = class {
	constructor(builder, listener) {
		this.builder = builder;
		this.listener = listener;
	}
	async loadEsbuildConfig() {
		for (const configFile of ["esbuild.config.json", ".esbuildrc.json"]) {
	...
	async buildApiSteps(steps) {
		...
		const getStepPath = (step) => {
			return step.filePath.replace(this.builder.projectDir, relativePath).replace(/(.*)\.(ts|js)$/, "$1.js").replace(/\\/g, "/");
		};
		const __dirname = path.dirname(fileURLToPath(import.meta.url));
		const file = fs.readFileSync(path.join(__dirname, "router-template.ts"), "utf-8").replace("// {{imports}}", steps.map((step, index) => `import * as route${index} from '${getStepPath(step)}'`).join("\n")).replace("// {{router paths}}", steps.map((step, index) => `'${step.config.method} ${step.config.path}': { stepName: '${step.config.name}', handler: route${index}.handler, config: route${index}.config }`).join(",\n"));
		...
		await esbuild.build(userConfig ? {
			...defaultConfig,
			...userConfig
		} : defaultConfig);
	...
	async build(step) {
		const entrypointPath = step.filePath.replace(this.builder.projectDir, "").replace(/(.*)\.(ts|js)$/, "$1.js");
