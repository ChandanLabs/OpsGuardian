import { cloudApiWsUrl } from "./cloud-api/endpoints.mjs";
import { cloudApi } from "./cloud-api/index.mjs";
import { Stream } from "@motiadev/stream-client-node";

//#region src/cloud/new-deployment/deploy.ts
const deploy = async (input) => {
	...
	if (!ci) {
		...
		const interval = setInterval(() => {
			...
			if (state) listener.onDeployProgress(state);
		}, 1e3);
		await new Promise((resolve) => {
			subscription.addChangeListener((item) => {
				if (item && ["failed", "completed"].includes(item.status)) {
					...
					if (item.status === "completed") listener.onDeployEnd({ output: item.outputs });
					client.close();
					resolve();
				}
			});
		...
		context.log("deploy-progress", (message) => message.tag("progress").append(`Deployment in progress... You can view the deployment at ${response?.deploymentUrl}`));
		client.close();
	}
};
