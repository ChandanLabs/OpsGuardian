//#region src/cloud/new-deployment/streams/deployment-stream.ts
const createDefaultDeploymentData = (deploymentId) => ({
	id: deploymentId,
	status: "idle",
	phase: null,
...
var DeploymentStreamManager = class {
	constructor(stream) {
		this.stream = stream;
	}
	...
	async updateDeployment(deploymentId, data) {
		...
		if (!current) await this.stream.set(deploymentId, "data", {
	...
	async updateBuildOutput(deploymentId, buildOutput) {
		...
		if (existingIndex >= 0) updatedBuild[existingIndex] = buildOutput;
		else updatedBuild.push(buildOutput);
	...
	async updateUploadOutput(deploymentId, uploadOutput) {
		...
		if (existingIndex >= 0) updatedUpload[existingIndex] = uploadOutput;
		else updatedUpload.push(uploadOutput);
...
//#endregion
export { DeploymentStreamManager };
//# sourceMappingURL=deployment-stream.mjs.map