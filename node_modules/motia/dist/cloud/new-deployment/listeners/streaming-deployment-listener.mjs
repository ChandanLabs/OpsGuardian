import { DeploymentStreamManager } from "../streams/deployment-stream.mjs";

//#region src/cloud/new-deployment/listeners/streaming-deployment-listener.ts
var StreamingDeploymentListener = class {
	...
	async updateStream(update) {
		if (!await this.streamManager.getDeployment(this.deploymentId)) return;
	...
	async onWarning(id, warning) {
		...
		await this.updateStream({ message: `Warning: ${warning}` });
	}
	async onBuildWarning(warning) {
		this.warnings.push(warning);
		await this.updateStream({ message: `Build warning: ${warning.message}` });
	}
...
//#endregion
export { StreamingDeploymentListener };
//# sourceMappingURL=streaming-deployment-listener.mjs.map