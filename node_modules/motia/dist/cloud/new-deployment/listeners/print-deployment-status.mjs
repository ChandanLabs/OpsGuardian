...
//#region src/cloud/new-deployment/listeners/print-deployment-status.ts
let spinnerIndex = 0;
const getStatusColor = (status) => {
	switch (status) {
		case "completed": return pc.green;
		case "failed": return pc.red;
		case "progress": return pc.yellow;
...
const printDeploymentStatus = (data, context) => {
	...
	if (data.cron.length > 0) data.cron.forEach((cronJob) => {
	...
	else context.log("deployment-status-cron-no-jobs", (message) => message.append(`[${pc.green("✓")}]    ↳ No cron jobs configured`));
...
//#endregion
export { printDeploymentStatus };
//# sourceMappingURL=print-deployment-status.mjs.map