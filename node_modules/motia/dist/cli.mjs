...
import { wrapAction } from "./utils/analytics.mjs";
...
program.command("version").description("Display detailed version information").action(() => {
...
program.command("create [name]").description("Create a new motia project").option("-t, --template <template>", "The template to use for your project").option("-p, --plugin", "Create a plugin project").option("-i, --interactive", "Use interactive prompts to create project").option("--skip-redis", "Skip Redis binary installation and use external Redis").action((projectName, options) => {
	...
	return handler(async (arg, context) => {
		const { createInteractive } = await import("./create/interactive.mjs");
...
program.command("rules").command("pull").description("Install essential AI development guides (AGENTS.md, CLAUDE.md) and optional Cursor IDE rules").option("-f, --force", "Overwrite existing files").action(handler(async (arg, context) => {
	const { pullRules } = await import("./create/pull-rules.mjs");
	await pullRules({
		force: arg.force,
		rootDir: process.cwd()
	}, context);
}));
program.command("generate-types").description("Generate types.d.ts file for your project").action(wrapAction(async () => {
	const { generateTypes } = await import("./generate-types.mjs");
	await generateTypes(process.cwd());
	process.exit(0);
}));
program.command("install").description("Sets up Python virtual environment and install dependencies").option("-v, --verbose", "Enable verbose logging").action(wrapAction(async (options) => {
	const { install } = await import("./install.mjs");
	await install({ isVerbose: options.verbose });
}));
program.command("dev").description("Start the development server").option("-p, --port <port>", "The port to run the server on", `${defaultPort}`).option("-H, --host [host]", "The host address for the server", `${defaultHost}`).option("-v, --disable-verbose", "Disable verbose logging").option("-d, --debug", "Enable debug logging").option("-m, --mermaid", "Enable mermaid diagram generation").option("--motia-dir <path>", "Path where .motia folder will be created").action(wrapAction(async (arg) => {
	if (arg.debug) {
...
program.command("start").description("Start a server to run your Motia project").option("-p, --port <port>", "The port to run the server on", `${defaultPort}`).option("-H, --host [host]", "The host address for the server", `${defaultHost}`).option("-v, --disable-verbose", "Disable verbose logging").option("-d, --debug", "Enable debug logging").option("--motia-dir <path>", "Path where .motia folder will be created").action(wrapAction(async (arg) => {
	if (arg.debug) {
		console.log("🔍 Debug logging enabled");
		process.env.LOG_LEVEL = "debug";
	}
...
program.command("emit").description("Emit an event to the Motia server").requiredOption("--topic <topic>", "Event topic/type to emit").requiredOption("--message <message>", "Event payload as JSON string").option("-p, --port <number>", "Port number (default: 3000)").action(wrapAction(async (options) => {
	const url = `http://localhost:${options.port || 3e3}/emit`;
...
docker.command("setup").description("Setup a motia-docker for your project").action(wrapAction(async () => {
	const { setup } = await import("./docker/setup.mjs");
	await setup();
	process.exit(0);
}));
docker.command("run").description("Build and run your project in a docker container").option("-p, --port <port>", "The port to run the server on", `${defaultPort}`).option("-n, --project-name <project name>", "The name for your project").option("-s, --skip-build", "Skip docker build").action(wrapAction(async (arg) => {
	const { run } = await import("./docker/run.mjs");
	await run(arg.port, arg.projectName, arg.skipBuild);
	process.exit(0);
}));
docker.command("build").description("Build your project in a docker container").option("-n, --project-name <project name>", "The name for your project").action(wrapAction(async (arg) => {
	const { build } = await import("./docker/build.mjs");
