...
const createInteractive = async (args, context) => {
	...
	if (args.plugin) {
		if (!args.name) {
			context.log("failed", (message) => message.tag("failed").append(`Project name is required: ${pc.bold("motia create --plugin [project-name]")}\n`));
			return;
		}
		return create({
	...
	} else if (!args.template) questions.push({
		type: "list",
		name: "template",
	...
	if (!args.name) questions.push({
		...
		validate: (input) => {
			if (input && input.trim().length > 0) {
				if (!/^[a-zA-Z0-9][a-zA-Z0-9-_]*$/.test(input.trim())) return "Project name must start with a letter or number and contain only letters, numbers, hyphens, and underscores";
			}
			return true;
		},
