import { StreamAdapter } from "../../interfaces/stream-adapter.interface.mjs";

//#region src/adapters/defaults/stream/memory-stream-adapter.d.ts
declare class MemoryStreamAdapter<TData> extends StreamAdapter<TData> {
  private state;
  constructor(streamName: string);
  init(): Promise<void>;
  getGroup<T>(groupId: string): Promise<T[]>;
  get<T>(groupId: string, id: string): Promise<T | null>;
  set<T>(groupId: string, id: string, value: T): Promise<T & {
    id: string;
  }>;
  delete<T>(groupId: string, id: string): Promise<T | null>;
  clear(groupId: string): Promise<void>;
  private _makeKeyPrefix;
  private _makeKey;
}
//#endregion
export { MemoryStreamAdapter };
//# sourceMappingURL=memory-stream-adapter.d.mts.map