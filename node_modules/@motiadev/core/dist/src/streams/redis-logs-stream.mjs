...
//#region src/streams/redis-logs-stream.ts
const LOG_TTL_SECONDS = 4320 * 60;
var RedisLogsStream = class extends StreamAdapter {
	constructor(client) {
		super("__motia.logs");
	...
	async delete(groupId, id) {
		...
		await Promise.all([this.client.del(key), this.send({
			groupId,
			id
		}, {
			type: "delete",
	...
	async getGroup(groupId) {
		...
		const logs = (await this.client.mGet(keys)).filter((v) => v !== null).map((v) => JSON.parse(v));
...
//#endregion
export { RedisLogsStream };
//# sourceMappingURL=redis-logs-stream.mjs.map